...
@detection_bp.route("/notification-logs", methods=["GET"])
@login_required
def notification_logs():
    """Recent email/SMS/call logs (newest first). Optional ?channel=email|sms|call."""
    from api.database_v2 import notification_logs_col
    limit = min(request.args.get("limit", 50, type=int), 200)
    channel = request.args.get("channel")
    query = {"channel": channel} if channel in ("email", "sms", "call") else {}
    logs = list(notification_logs_col().find(query).sort("timestamp", -1).limit(limit))
    for l in logs:
        l["_id"] = str(l["_id"])
        if isinstance(l.get("timestamp"), datetime):
            l["timestamp"] = l["timestamp"].isoformat()
    return jsonify({"success": True, "data": logs, "error": None})
